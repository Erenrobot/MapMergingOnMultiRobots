...
cam=(332.9375, 187.105)

K2 = np.eye(3)
K2[0, 0] = K2[1, 1] = 100
K2[0, 2] = img2.shape[1] * 0.5
K2[1, 2] = img2.shape[0] * 0.5
cam_matrix=[[300,0,300],[0,300,300],[0,0,1]]
...
for i in range(0,32):
    ...
    for j in range(0,len(grup[i])):
        ...
        if(medyan_farki<=1 and medyan_farki>=-1):
            ...
            pts2.append(keypoints_2[grup[i][j].trainIdx].pt)
            pts1.append(keypoints_1[grup[i][j].queryIdx].pt)
...
E,mask=cv2.findEssentialMat(pts1, pts2, K, method=cv2.RANSAC, prob=0.999,threshold=1)
_, R, t, mask = cv2.recoverPose(E, pts1, pts2, K, pp = cam)
r = Rotation.from_matrix(R)
rz, ry, rx = r.as_euler("zyx", degrees=True)
print("Rotation x:"+str(rx)+" Rotation y:"+str(ry)+" Rotation z:"+str(rz))
